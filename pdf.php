...
$funcion->conectar();
	$sql="SELECT jornadas.numjornada, jornadas.fechajornada, jornadas.direccion, jornadas.tlfcontacto, jornadas.descripcion, parroquia.nombreparro FROM jornadas INNER JOIN parroquia ON jornadas.numparroquia=parroquia.numparroquia";
	$resSql=$funcion->consulta($sql);
$pdf = new Cezpdf('A4');
$pdf->selectFont('fonts/Helvetica.afm');
	
	while($row=mysql_fetch_row($resSql)){
		$data[]=array('numjornada'=>$row[0], 'fechajornada'=>$row[1],'direccion'=>$row[2],'tlfcontacto'=>$row[3],'descripcion'=>$row[4],'nombreparro'=>$row[5]);
	}
